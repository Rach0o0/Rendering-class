...
async function loadHDRToTexture(device, url) {
    return new Promise((resolve, reject) => {
        ...
        hdr.onload = () => {
            ...
            // Données RGBE (Uint8Array) fournies par hdrpng.js
            const dataRGBE = hdr.dataRGBE; // longueur = width * height * 4
            ...
            device.queue.writeTexture(
                ...
                dataRGBE,
                {
                    bytesPerRow: width * 4,       // 4 octets/pixel
                    rowsPerImage: height,
                },
...
async function main()
{
    ...
    //______________________________________
    const { texture: envTexture } = await loadHDRToTexture(
        device,
        "luxo_pxr_campus.hdr.png"
    );
    ...
    // UNIFORM BUFFER
    //______________________________________
    let bytelength = 7 * 16; // Buffers are allocated in vec4 chunks
    ...
    // PARAMETERS
    //______________________________________
    const eye = vec3(0.0, 1.5, 6.0);
    const p   = vec3(0.0, 1.0, 0.0);
