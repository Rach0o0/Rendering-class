...
fn decode_rgbE(rgba: vec4u) -> vec3f {
    // format RGBE classique : rgb 0..255, e 0..255
    if (rgba.w == 0u) {
        return vec3f(0.0);
    }
...
fn sample_env(dir: vec3f) -> vec3f {
    ...
    let dims = textureDimensions(envMap);              // vec2<u32>
    let size_f = vec2f(f32(dims.x), f32(dims.y));

    var st = uv * size_f;
    st = clamp(st, vec2f(0.0), size_f - vec2f(1.0));   // évite l’out-of-range

    let ij = vec2u(st);                                // coordonnées entières
    let texel : vec4u = textureLoad(envMap, ij, 0);    // lecture RGBE
...
fn intersect_plane_y0(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
    ...
    (*hit).shader = 99u; // holdout shader ID
...
fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, isPrimary: bool) -> bool {
    ...
    */
    ...
    // Try plane
    var hit_plane : HitInfo;
    reset_hit(&hit_plane, r_box.tmax);
    var r_plane = r_box;
    let got_plane = intersect_plane_y0(&r_plane, &hit_plane);

    if (got_plane && (!got_tri || hit_plane.dist < hit_tri.dist)) {
        (*hit) = hit_plane;
        (*r).tmax = hit_plane.dist;
        return true;
    }
...
fn sample_area_light(hitPos: vec3f, hitN: vec3f, t: ptr<function, u32>) -> Light {
    ...
    let s = 10.0;
    let v0 = attribs[face.x*2u + 0u].xyz *s;
    let v1 = attribs[face.y*2u + 0u].xyz *s ;
    let v2 = attribs[face.z*2u + 0u].xyz *s;
...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
    ...
    if (n_dot_l > 0.0) {
        var shadowRay = Ray((*hit).position + 0.001 * n, l, 0.001, 1e9);
        var shadowHit: HitInfo;
        reset_hit(&shadowHit, 1e9);

        if (!intersect_scene(&shadowRay, &shadowHit, false)) {
            Lo += ((*hit).diffuse / 3.14159) * sun.L_i * n_dot_l;
        }
    }
...
fn holdout_AO(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function,u32>) -> vec3f {
    var occlusion = 0.0;
    let samples = 16u;
    ...
    for (var i = 0u; i < samples; i++) {
        ...
        var rr = Ray(hit.position + hit.normal * 0.001, wi, 0.001, 1e9);
        var h2 : HitInfo;
        reset_hit(&h2, 1e9);

        if (intersect_scene(&rr, &h2, false)) {
            occlusion += 1.0;
        }
    }
...
fn holdout_shadow_catcher(viewDir: vec3f, hitPos: vec3f, hitN: vec3f) -> vec3f {
    let base = sample_env(viewDir);

    let sun = sample_directional_light();
    let l = normalize(sun.w_i);
    let n = normalize(hitN);

    var shadow = 1.0;
    if (dot(n, l) > 0.0) {
        var shadowRay = Ray(hitPos + 0.001 * n, l, 0.001, 1e9);
        var h: HitInfo;
        reset_hit(&h, 1e9);

        if (intersect_scene(&shadowRay, &h, false)) {
            shadow = 0.15;
        }
    }
...
fn shade(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t:ptr<function, u32>) -> vec3f {
    switch u32(uniforms.objectShader) {
        ...
        case 99u: { return holdout_shadow_catcher((*r).direction, hit.position, hit.normal); }
...
{   
    ...
    */
    ...
    for (var k = 0; k < n; k++) {
        ...
        */
        // debug: montrer si l’AABB coupe trop
        /*if (!intersect_aabb(&r)) {
            return vec4f(1,0,0,1); // rouge = rejeté
        }*/
