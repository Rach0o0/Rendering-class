...
fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) ->bool {
    ...
    let plane_shader = u32(uniforms.shaderSelect.y);
    ...
    let sphere_shader = u32(uniforms.shaderSelect.x);
    ...
    let tri_shader = u32(uniforms.shaderSelect.y);
...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f {
    ...
    var shadow_hit = HitInfo(false, 1e9, vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), 0u, false, 1.0);
...
fn main_fs(@location(0) coords: vec2f) -> @location(0) vec4f
{
    ...
    for (var depth = 0; depth < 5; depth++){
        hit = HitInfo(false, 1e9, vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), SHADER_COLOR, false, 1.0);
