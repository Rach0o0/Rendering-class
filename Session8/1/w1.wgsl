...
struct Uniforms {
    ...
    frame: f32, progressive: f32, bgFlag: f32, _pad6: f32
}; 
...
const SHADER_TRANSPARENT : u32 = 6u;
...
fn transparent(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
    let n_geom = normalize((*hit).normal);
    let d = normalize((*r).direction);
    ...
    // on suppose d'abord qu'on vient de l'air et qu'on entre dans le verre
    var normal = n_geom;
    var n_i = n_air;
    var n_t = n_glass;

    // cos(theta_i) = angle entre -d et la normale sortante
    var cos_i = dot(-d, normal);

    // si cos_i < 0, on est "dans" le matériau, on inverse
    if (cos_i < 0.0) {
        cos_i = -cos_i;
        normal = -n_geom;
        n_i = n_glass;
    ...
    // Snell : test de TIR + cos_t
    let k = 1.0 - eta * eta * (1.0 - cos_i * cos_i);

    if (k < 0.0) {
        // Total Internal Reflection : réflection pure, R = 1
...
fn shade(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t:ptr<function, u32>) -> vec3f {
    ...
    } else if (hit.shader == SHADER_TRANSPARENT) {
...
{   
    ...
    */
    ...
    var accum_color = final_color;
    if (uniforms.progressive == 1.0) {
        let prev_sum = textureLoad(renderTexture, vec2u(fragcoord.xy), 0).rgb * uniforms.frame;
        accum_color = (final_color +prev_sum)/(uniforms.frame + 1.0);
    }
