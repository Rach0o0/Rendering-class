...
@group(0) @binding(7) var<storage> bspTree    : array<u32>;
...
fn intersect_bsp(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
    var branch_node: array<u32, 2u * MAX_LEVEL>;
    var branch_ray : array<f32, 2u * MAX_LEVEL>;
    var branch_lvl : u32 = 0u;
    var node : u32 = 0u;

    for (var i: u32 = 0u; i <= MAX_LEVEL; i = i + 1u) {
        let packed = bspTree[node*4u + 0u];
        let axis_leaf = packed & 3u;

        if (axis_leaf == BSP_LEAF) {
            let count = packed >> 2u;
            var found = false;
            let baseId = bspTree[node*4u + 1u];
            for (var j: u32 = 0u; j < count; j = j + 1u) {
                let prim = treeIds[baseId + j];
                let matIdx = matIndices[prim];
                let mat = materials[matIdx];
                if (intersect_triangle(r, hit, prim, mat.emission, mat.color, vec3f(0.0, 0.0, 0.0), 1u)) {
                    (*r).tmax = (*hit).dist;
                    found = true;
                }
            }
            ...
            branch_lvl -= 1u;
            i = branch_node[branch_lvl*2u + 0u];
            node = branch_node[branch_lvl*2u + 1u];
            (*r).tmin = branch_ray[branch_lvl*2u + 0u];
            (*r).tmax = branch_ray[branch_lvl*2u + 1u];
            continue;
        }

        var axis_dir : f32;
        var axis_org : f32;
        if (axis_leaf == BSP_X_AXIS) {
            axis_dir = (*r).direction.x;
            axis_org = (*r).origin.x;
        } else if (axis_leaf == BSP_Y_AXIS) {
            axis_dir = (*r).direction.y;
            axis_org = (*r).origin.y;
        } else {
            axis_dir = (*r).direction.z;
            axis_org = (*r).origin.z;
        }

        let leftIdx  = bspTree[node*4u + 2u];
        let rightIdx = bspTree[node*4u + 3u];
        var near_node : u32;
        var far_node  : u32;
        if (axis_dir >= 0.0) {
            near_node = leftIdx;
        ...
        let denom = select(axis_dir, 1e-12, abs(axis_dir) < 1e-12);
        let t = (plane - axis_org) / denom;
        ...
        if (t > (*r).tmax) {
            ...
            branch_node[branch_lvl*2u + 0u] = i;
            branch_node[branch_lvl*2u + 1u] = far_node;
            branch_ray[branch_lvl*2u + 0u] = t;
            branch_ray[branch_lvl*2u + 1u] = (*r).tmax;
            branch_lvl += 1u;
            (*r).tmax = t;
            node = near_node;
        }
...
fn sample_area_light(hitPos: vec3f, hitN: vec3f) -> Light {
    let lightPos = vec3f(0.0, 1.0, 0.0);
    let lightNormal = vec3f(0.0, -1.0, 0.0);
    let lightArea = 4.0;

    let wi = lightPos - hitPos;
    let dist2 = dot(wi, wi);
    let dist = sqrt(dist2);
    let dir = wi / dist;

    let cos_i = max(dot(hitN, dir), 0.0);
    let cos_o = max(dot(-dir, lightNormal), 0.0);

    let I = vec3f(60.0, 60.0, 60.0);
    let L_i = I * (cos_i * cos_o / (3.14159 * dist2)) * lightArea;
